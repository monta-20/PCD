...
const TINV = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5001/tinv");
        setAb(res.data);
      } catch (err) {
        console.log(err);
      }
      ...
      try {
        const ress = await axios.get("http://localhost:5001/tinv_forecast");
  ...
  return (
    <>
      <canvas ref={chartRef} />
      <Score  title="TUNINVEST - SICAR" ord={ab.slice(0, 50).map((item) => item.date.split('T')[0])}
