...
const OTH = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5001/oth");
        setAb(res.data);
      } catch (err) {
        console.log(err);
      }
      ...
      try {
        const ress = await axios.get("http://localhost:5001/oth_forecast");
  ...
  return (
    <>
      <canvas ref={chartRef} />
      <Score  title="ONE TECH" ord={ab.slice(0, 50).map((item) => item.date.split('T')[0])}
