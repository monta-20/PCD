...
const SOKNA = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5001/sokna");
        setAb(res.data);
      } catch (err) {
        console.log(err);
      }
      ...
      try {
        const ress = await axios.get("http://localhost:5001/sokna_forecast");
